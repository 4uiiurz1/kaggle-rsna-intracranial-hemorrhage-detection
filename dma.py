...
def parse_args():
    ...
    parser.add_argument('--k', default='19,19,9,15,13,19')
    parser.add_argument('--h', default='2.60,0.55,0.50,0.95,1.25,0.65')
...
def main():
    ...
    hs = [float(e) for e in test_args.h.split(',')]
    ws = []
    for k, h in zip(ks, hs):
        if test_args.kernel == 'gaussian':
            w = np.exp(-np.arange(-k // 2 + 1, k // 2 + 1)**2 / (2 * h**2))
        elif test_args.kernel == 'exponential':
            w = np.exp(-np.abs(np.arange(-k // 2 + 1, k // 2 + 1)) / h)
        else:
    ...
    test_df[['ID', 'Label']].to_csv(
        'submissions/%s_%s_%s.csv' %(test_args.name, test_args.kernel,
        '_'.join(['k%ds%.2f' %(k, h) for k, h in zip(ks, hs)])), index=False)
